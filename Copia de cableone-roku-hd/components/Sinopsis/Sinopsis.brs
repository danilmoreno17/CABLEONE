...
'    'get titulo del contenido
   ...
   ' m.btnSeasons = m.top.findNode("btnSeasons")
   ' m.btnSeasons.text = tr(m.LABELSeason)    
...
'    'Eventos
    ...
    m.seasonList.observeField("itemSelected", "showEpSinopsis")    
    m.sinopsisEP = m.top.findNode("sinopsisEP")
'    
...
Sub onVisibleChange()
    ...
    if m.top.visible = true then
        ...
        m.seasonList.content=invalid
         ...
         toggleView(0)
         if(m.description.seasonList<>invalid) then
            m.description.seasonList.observeField("itemSelected", "showSeason")
         end if 
    else
...
sub showEpSinopsis()
    ...
    m.sinopsisEP.content = m.seasonList.content.getChild(m.seasonList.itemFocused)
    stop
    ShowScreen(m.sinopsisEP)
end sub
...
Sub OnContentChange()
?"onContentChange()" +m.top.content.mediaType
    if(m.top.content.mediaType="serie") then
        m.btnWatch.text     =tr(m.LABELSeason)
    else
        m.btnWatch.text     = tr(m.LABELWatch)
    end if
    m.description.providers = ["VUDU","Netflix"]  
    
    m.description.content   = m.top.content
    'TODO integrar con parametros
    ...
    if(m.top.content.mediaType="serie") then
        ...
        createTask("season",m.top.content.id,"OnSeasonCompleted","99")
    end if 
    mostrarControles()
End Sub

function OnSeasonCompleted()
?"OnIndexChanged"
...
sub mostrarControles()
 if(m.top.content<>invalid) then
        if(m.top.content.mediaType="movie" ) then
             m.btnWatch.text      =tr(m.LABELwatch)
            'm.btnFavorite.visible   =false
            'm.btnSeasons.visible = false
        end if    
        if(m.top.content.mediaType="serie" ) then
            m.btnWatch.text      =tr(m.LABELSeason)
            'm.btnFavorite.visible   =false
            'm.btnWatch = m.btnSeasons
        end if    
    end if
end sub
...
? index
    ...
    if(index=0) then
        ...
        if(index=1) then
            cerrarOverview()
            if(m.btnWatch.text=m.Labelseason) then
                abrirSeasons()
            else
                showDeeplinking()
            end if
...
sub cerrarDeeplinking()
    m.productList.visible="false"
    m.productList.width="10"
end sub

sub createTask(mediaType as String, serieid as String, callback as String, permission as String )
    m.loadingIndicator = m.top.content.createChild("icono_cargando")
    m.simpleTask = CreateObject("roSGNode", "tskGetContentData")
    if(m.top.providers<>invalid) then
        m.simpleTask.provider= m.top.providers
    else
        m.simpleTask.provider= []
    end if
